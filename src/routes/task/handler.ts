...
const postTaskHandler = async (request: FastifyRequest, reply: FastifyReply) => {
	...
	const isSubscribed = await prisma.list.findUnique({ where: { id: listId, subscribers: { some: { id: request.user.id } } } });
	if (!isSubscribed)
